...
int main( int argc, char ** argv ) {
  ...
  ssmocma.init( mocmaes );
  //###end<optimizer>

  //###begin<loop>
  // Iterate the optimizer
  while( mocmaes.evaluationCounter() < 5000) {
    ssmocma.step( mocmaes );
  }
  //###end<loop>

  for( std::size_t i = 0; i < ssmocma.solution().size(); i++ ) {
      mocmaes.calcPathLength(ssmocma.solution()[i].point);
      for(std::size_t j= 0; j < mocmaes.numberOfObjectives(); j++ ){
        outfile<<ssmocma.solution()[ i ].value[j]<<" ";
      }
      outfile<<endl;
  }
  outfile.close();
